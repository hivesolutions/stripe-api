...
class ChargeAPI(object):
    ...
    def create_charge(
        ...
        if address_country:
            params["source[address_country]"] = address_country
        if address_state:
            params["source[address_state]"] = address_state
        if address_city:
            params["source[address_city]"] = address_city
        if address_zip:
            params["source[address_zip]"] = address_zip
        if address_line1:
            params["source[address_line1]"] = address_line1
        if address_line2:
            params["source[address_line2]"] = address_line2
    ...
    def create_charge_token(
        ...
        url = self.base_url + "charges"
        params = {"amount": amount, "currency": currency, "source": token}
        if description:
            params["description"] = description
        if statement_descriptor:
            params["statement_descriptor"] = statement_descriptor[:22]
