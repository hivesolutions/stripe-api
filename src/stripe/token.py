...
class TokenAPI(object):
    ...
    def create_token(
        ...
        if cvc:
            params["card[cvc]"] = cvc
        if name:
            params["card[name]"] = name
        if address_country:
            params["card[address_country]"] = address_country
        if address_state:
            params["card[address_state]"] = address_state
        if address_city:
            params["card[address_city]"] = address_city
        if address_zip:
            params["card[address_zip]"] = address_zip
        if address_line1:
            params["card[address_line1]"] = address_line1
        if address_line2:
            params["card[address_line2]"] = address_line2
