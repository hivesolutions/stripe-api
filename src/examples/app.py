...
class StripeApp(appier.WebApp):
    ...
    def __init__(self, *args, **kwargs):
        appier.WebApp.__init__(self, name="stripe", *args, **kwargs)
    ...
    def new_card_customer(self, id):
        ...
        api = self.get_api()
        balance = api.create_card_customer(id, exp_month, exp_year, number)
        return balance
    ...
    def new_charge(self):
        ...
        balance = api.create_charge(
            amount, currency, exp_month, exp_year, number, cvc=cvc, name=name
        )
        return balance
    ...
    def new_token(self):
        ...
        name = self.field("name", None)
        api = self.get_api()
        token = api.create_token(exp_month, exp_year, number, cvc=cvc, name=name)
        return token
    ...
    def new_3d_secure(self):
        ...
        return_url = return_url or self.url_for(
            "stripe.return_3d_secure", absolute=True
        )
        token = api.create_token(exp_month, exp_year, number, cvc=cvc, name=name)
        secure = api.create_3d_secure(amount, currency, return_url, card=token["id"])
        redirect_url = secure.get("redirect_url", None)
        if redirect and redirect_url:
            return self.redirect(
                self.url_for("stripe.redirect_3d_secure", redirect_url=redirect_url)
            )
        return secure
    ...
    def return_3d_secure(self):
        ...
        return dict(id=id, status=status, error_code=error_code)
    ...
    def new_source(self):
        ...
        name = self.field("name", None)
        api = self.get_api()
        return_url = return_url or self.url_for("stripe.return_source", absolute=True)
        source = api.create_card_source(exp_month, exp_year, number, cvc=cvc, name=name)
        secure = api.create_3d_secure_source(
            amount, currency, return_url, card=source["id"]
        )
    ...
    def return_source(self):
        ...
        return dict(client_secret=client_secret, source=source, livemode=livemode)
